...
async def _(event):
    ...
    shared = event.pattern_match.group(2)
    if shared:
        shared = True
    elif not shared:
        shared = False
    ...
    if required_file_name:
        ...
        try:
            g_drive_link = await upload_file(http, required_file_name, file_name, mime_type,mone,parent_id, shared)
...
async def _(event):
    ...
    if os.path.isdir(input_str):
        ...
        dir_id = await create_directory(http, os.path.basename(os.path.abspath(input_str)), parent_id, shared)
        await DoTeskWithDir(http, input_str, event, dir_id, shared)
...
async def DoTeskWithDir(http, input_directory, event, parent_id, shared_drive=None):
    ...
    for a_c_f_name in list_dirs:
        ...
        if os.path.isdir(current_file_name):
            ...
            r_p_id = await DoTeskWithDir(http, current_file_name, event, current_dir_id, shared_drive)
        else:
            file_name, mime_type = file_ops(current_file_name)
            # current_file_name will have the full path
            g_drive_link = await upload_file(http, current_file_name, file_name, mime_type, event, parent_id, shared_drive)
            r_p_id = parent_id
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id, shared_drive=False):
    ...
    # Insert a file
    file = drive_service.files().insert(body=body, media_body=media_body, supportsAllDrives=True)
    response = None
    display_message = f"Uploading to G-Drive:\nFile Name: `{file_name}`"
    await event.edit(display_message)
